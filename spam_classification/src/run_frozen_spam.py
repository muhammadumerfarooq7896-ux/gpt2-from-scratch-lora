...
# ---------------------------------------------------------------------------
# Dataset (identical to the LoRA script - same data, same tokenization)
# ---------------------------------------------------------------------------
...
def build_frozen_model():
    ...
    # Step 1: freeze EVERYTHING currently in the model (the pretrained backbone)
    for param in model.parameters():
        param.requires_grad = False

    # Step 2: attach a FRESH classification head - created after freezing, so
    # it defaults to requires_grad=True and is the only trainable part
...
def save_head_checkpoint(model, path):
    """
    Saves ONLY the classification head - the sole trainable part in this
    strategy. Even smaller than the LoRA checkpoint, since there's no A/B
    matrices at all, just the head's weight and bias.
    """
...
def main():
    ...
    checkpoint_path = os.path.join(CHECKPOINT_DIR, "frozen_spam.pt")
