"""
LoRA fine-tuning for spam classification.

Strategy: freeze GPT-2's pretrained backbone, inject LoRA (from-scratch) into
every linear layer, then attach a FRESH classification head on top. Only the
LoRA A/B matrices and the new head train - the original GPT-2 weights never move.
...
from lora import apply_lora, count_trainable_params
...
class SpamDataset(Dataset):
    ...
    GPT-2's <|endoftext|> token as padding.
...
def build_lora_model():
    ...
    # Step 1: freeze the pretrained backbone + inject LoRA into every linear layer
    apply_lora(model, rank=LORA_RANK, alpha=LORA_ALPHA)

    # Step 2: attach a FRESH classification head - created after freezing, so it
    # defaults to requires_grad=True and is untouched by LoRA wrapping
...
def save_lora_checkpoint(model, path):
    """
    Saves ONLY the parameters that were actually trained - the LoRA A/B
    matrices and the classification head - not the frozen GPT-2 backbone.
    ...
    print(f"Saved LoRA + head checkpoint to {path} ({size_mb:.2f} MB, "
...
def train_one_epoch(model, loader, optimizer, device):
    ...
    for input_ids, labels in loader:
        ...
        logits = model(input_ids)[:, -1, :]  # last position's logits -> class scores
...
def main():
    ...
    checkpoint_path = os.path.join(CHECKPOINT_DIR, "lora_spam.pt")
    ...
    print(f"\nSaved metrics to {os.path.join(RESULTS_DIR, 'lora_metrics.json')}")
