...
MAX_LENGTH = 128  # movie reviews are longer than spam texts - truncated to
                   # the first 128 tokens for T4 speed, same trade-off as
                   # the other two IMDB scripts
...
def build_full_finetune_model():
    ...
    # No freezing - every existing parameter stays trainable by default.
...
def save_full_checkpoint(model, path):
    """Saves the entire model state dict - every parameter changed during
    full fine-tuning, so there's no smaller subset to save (unlike frozen/LoRA)."""
...
def main():
    ...
    checkpoint_path = os.path.join(CHECKPOINT_DIR, "full_finetune_imdb.pt")
