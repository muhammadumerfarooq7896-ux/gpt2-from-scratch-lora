...
class IMDBDataset(Dataset):
    """
    Tokenizes each review, truncates/right-pads to MAX_LENGTH. Since GPT-2
    is causal, the LAST position's representation after right-padding still
    reflects the whole real input, so logits[:, -1, :] is safe to use
    directly as the classification logits.
    """
...
# ---------------------------------------------------------------------------
# Model setup - same critical ordering as run_lora_spam.py:
# apply_lora() FIRST (freezes backbone + injects LoRA), THEN attach the head.
# ---------------------------------------------------------------------------
...
def save_lora_checkpoint(model, path):
    """
    Saves ONLY the trained parameters (LoRA A/B matrices + head), not the
    frozen GPT-2 backbone. Same efficiency story as the spam checkpoint -
    a few MB instead of ~500MB.
    """
...
def main():
    ...
    checkpoint_path = os.path.join(CHECKPOINT_DIR, "lora_imdb.pt")
